...
// perform a safe memory allocation.
...
int read_tree(node *nodes[], int num_lines) {
    ...
    for (i = 0; i < num_lines; i++) {
        ...
        for (j = 0; j < num_children; j++) {
            ...
            // check if children node already exists.
...
// compute number of descendants from one node.
int score_one(node *n, int d) {
    ...
    for (i = 0; i < n->num_children; i++) { // recursive call.
...
// compute number of descendants from all nodes.
